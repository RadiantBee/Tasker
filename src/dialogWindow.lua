...
function EmptyDiaolgWindow(x, y, width, height)
    ...
    emptyDiaolgWindow.toggleActive = function (self) self.active = not self.active end
...
function CreateDialogWindowNewTask(x, y, width, height, taskList)
    ...
    newTaskDialogWindow.draw = function (self, mouseX, mouseY)
        ...
        if self.active then
            ...
            self.saveButton:draw(self.x + self.width/2 - self.saveButton.width/2,self.y + self.height + self.titleHeight - self.saveButton.height - 10,mouseX, mouseY,5,3)
        end
    end
