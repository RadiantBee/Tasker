...
local function Task(argTable, taskList)
    ...
    task.typeColors = {{0.5,0.5,0.5},{0.8,0,0},{0,0.8,0},{0.5,0.5,1},{0.8,0.8,0},{0.8,0,0.8}}
    ...
    task.activeButton = Button("V", function(self)
        ...
        if self.activeButton.text == "V" then
    ...
    task.deleteButton = Button("X", function (argTable)
        if argTable[2].readyToDelete then
            for id, task in pairs(argTable[1].tasks) do
                if task.title == argTable[2].title and
                    task.description1 == argTable[2].description1 and
                    task.description2 == argTable[2].description2 and
                    task.description3 == argTable[2].description3 and
                    task.description4 == argTable[2].description4 and
                    task.type == argTable[2].type then
    ...
    task.draw = function (self,mouseX,mouseY)
        ...
        if self.active then
            ...
            self.deleteButton:draw(self.x + self.width - self.deleteButton.width, self.y + self.titleHeight + self.height - self.deleteButton.height, mouseX, mouseY,6,3)
            self.editButton:draw(self.x, self.y + self.titleHeight + self.height - self.deleteButton.height, mouseX, mouseY,6,3)
            self.colorChangeButton:draw(self.x + self.width/2 - self.deleteButton.width/2, self.y + self.titleHeight + self.height - self.deleteButton.height, mouseX, mouseY,6,3)
        end
        love.graphics.setColor(0,0,0)
...
local function TaskList()
    ...
    taskList.loadFromFile = function (self, file)
        ...
        if not taskFile then error("Cannot acces the save file") end
    ...
    taskList.saveToFile = function (self, file)
        ...
        for _, task in pairs(self.tasks) do
            taskFile:write(task.title.."|"..task.description1.."|"..task.description2.."|"..
            task.description3.."|"..task.description4.."|"..task.type.."|"..task.x.."|"..task.y..
            "|"..task:writeActiveness().."|"..task.typeColorId.."\n")
         end
        taskFile:close()
    end
